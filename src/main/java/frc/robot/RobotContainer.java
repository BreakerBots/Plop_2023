...
public class RobotContainer {
    ...
    private static double speedCoefficient = MAX; // to lazy to actually use slow mode
    ...
    private static final BreakerTeleopSwerveDriveController teleopDriveController = 
      new BreakerTeleopSwerveDriveController(drivetrainSys, 
        () -> driverControllerSys.getLeftThumbstick().getY() * speedCoefficient, 
        () -> driverControllerSys.getLeftThumbstick().getX() * speedCoefficient, 
        () -> driverControllerSys.getRightThumbstick().getX() * speedCoefficient
        );
  ...
  private void configureBindings() {
    ...
    driverControllerSys.getDPad().getRight().onTrue(new InstantCommand(() -> {
      if (speedCoefficient == MAX) {
        speedCoefficient = Constants.DriveConstants.SLOW_MODE_LINEAR_MULTIPLIER;
      } else {
        speedCoefficient = 1;
      }
      for (int i = 0; i < 10; i++) {
        System.out.println("SPEED COEFF: " + speedCoefficient);
      }
    }));
